    ...
    const fetchAIPlan = async () => {
      try {
        ...
        if (res.ok) {
          ...
          // Extract specifically the transfer count for WARD B
          // Backend sends: "action_plan_transfers": { "ward_b": X, ... }
  ...
  return (
    <div style={{ padding: 28, maxWidth: 1200, margin: '24px auto', color: '#0f172a', fontFamily: 'Inter, sans-serif' }}>
      ...
      <header style={{ marginBottom: 32, borderBottom: '1px solid #e2e8f0', paddingBottom: 16 }}>
        <h1 style={{ fontSize: 32, fontWeight: 800, margin: 0, color: '#1e293b' }}>Ward B Portal</h1>
              ...
              {incomingCount > 0 ? (
                ...
                <div style={{ background: '#fff7ed', padding: 16, borderRadius: 12, marginBottom: 20 }}>
                  ...
                  <p style={{ margin: 0, fontSize: 13, color: '#c2410c', fontStyle: 'italic' }}>
                    "AI has identified {incomingCount} stable patients suitable for Ward B based on current ETU load."
                  </p>
                </div>
              ) : (
                // --- NO REQUESTS ---
