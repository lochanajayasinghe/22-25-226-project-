...
const Ward_G_NurseDashboard = () => {
  ...
  useEffect(() => {
    const fetchWardStatus = async () => {
      try {
        ...
        const response = await fetch('http://localhost:5001/api/ward-status/GEN');
  ...
  // --- 2. FETCH AI OPTIMIZATION PLAN (GENERAL WARD) ---
  useEffect(() => {
    const fetchAIPlan = async () => {
      try {
        ...
        if (res.ok) {
          ...
          // Extract count specifically for GENERAL WARD ('general' key from backend)
              ...
              <>
                {incomingCount > 0 ? (
                  ...
                  <div style={{ display: 'flex', gap: '32px', alignItems: 'center', background: 'white', padding: '24px', borderRadius: '16px', border: '1px solid #fed7aa', boxShadow: '0 4px 6px -1px rgba(249, 115, 22, 0.1)' }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <p style={{ fontSize: '15px', color: '#334155', lineHeight: '1.5' }}>
                        AI has identified <strong>{incomingCount} stable patients</strong> in ETU suitable for General Ward. 
                  ...
                  <div style={{ background: '#f0fdf4', padding: '32px', borderRadius: '16px', border: '1px solid #dcfce7', textAlign: 'center' }}>
                    ...
                    <p style={{ color: '#15803d', marginTop: '8px' }}>General Ward is optimized. No immediate transfers required from ETU.</p>
                  </div>
                )}
              </>
            )}
            ...
            {loadingStats ? (
              ...
              <div style={styles.statGrid}>
                ...
                <div style={styles.statBox}>
                  ...
                  <p style={{ fontSize: '12px', color: '#64748b', marginTop: '4px' }}>Total Registered Beds</p>
                </div>
                ...
                <div style={styles.statBox}>
                  ...
                  <p style={{ fontSize: '12px', color: '#64748b', marginTop: '4px' }}>Functional & Ready</p>
                </div>
              </div>
            )}
          </div>
