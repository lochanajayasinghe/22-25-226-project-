...
//         <p style={{ color: '#64748b', marginTop: 8 }}>Select a ward to input daily shift data.</p>
//       </div>
...
//   // ADDED 'occupied' to state
//   const [data, setData] = useState({ 
//     admissions: 0, 
//     discharges: 0, 
//     transfersOut: 0, 
//     deaths: 0,
//     occupied: 0 // Default 0
//   });
...
//       const isETU = ward.id === 'ETU'; // Check if the selected card is ETU

//       // --- DYNAMIC PAYLOAD CONSTRUCTION ---
//       // If ETU: use "ETU_" prefix (for your AI model)
//       // If Ward A/B/Gen: use generic names (Admissions, OccupiedBeds)
//       const payload = {
//         Date: date,
//         Shift_ID: shift,
//         Ward_ID: ward.id,       
...
//         // Dynamic Keys based on Ward ID
//         [isETU ? 'ETU_Admissions' : 'Admissions']: data.admissions,
//         [isETU ? 'ETU_Discharges' : 'Discharges']: data.discharges,
//         [isETU ? 'ETU_OccupiedBeds' : 'OccupiedBeds']: data.occupied, // Uses Manual Input
...
//         // Add capacity for reference
//         [isETU ? 'ETU_BedCapacity' : 'BedCapacity']: 25 
//       };
...
//             <p style={{ margin: '4px 0 0', color: '#64748b', fontSize: 14 }}>Daily Data Entry</p>
//           </div>
...
//             {/* CURRENT OCCUPANCY INPUT (Manually Editable) */}
//             <div style={{ gridColumn: 'span 2' }}>
//                 <div style={{ background: '#ecfdf5', padding: 12, borderRadius: 12, border: '1px solid #d1fae5' }}>
//                     <label style={{ display: 'block', fontSize: 12, fontWeight: 700, color: '#059669', textTransform: 'uppercase', marginBottom: 4 }}>
...
//               type="submit" 
//               disabled={loading}
//               style={{ flex: 2, padding: '12px', borderRadius: 12, border: 'none', background: '#0f172a', color: 'white', fontWeight: 600, cursor: loading ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}
//             >
...
const DailyInputModal = ({ ward, onClose }) => {
  ...
  const [wardCapacity, setWardCapacity] = useState(0); // DYNAMIC CAPACITY STATE
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        // --- 3. SEND DYNAMIC CAPACITY ---
        [isETU ? 'ETU_BedCapacity' : 'BedCapacity']: wardCapacity 
      };

      console.log("📤 Sending Payload with Dynamic Capacity:", payload);
