...
const Ward_A_NurseDailyInput = () => {
  ...
  // --- WARD DATA ---
  const wardInfo = { id: 'WARD-A', name: 'Ward A (Medical)', icon: Stethoscope, color: '#3b82f6' };

  // --- 1. FETCH LATEST SURGE STATUS ON MOUNT ---
  const fetchSurgeStatus = async () => {
    try {
      const response = await fetch('http://localhost:5001/api/get-history?type=SurgeUpdate&ward=WARD-A'); 
      if (response.ok) {
  ...
  const saveSurgeData = async (count) => {
    const payload = {
      ...
      EventType: 'SurgeUpdate' // This tells Backend to save to BedSurgeArea
    };
  ...
  return (
    <div style={{ padding: 40, fontFamily: 'Inter, sans-serif', background: '#f8fafc', minHeight: '100vh' }}>
      ...
      <div style={{ marginBottom: 40 }}>
        ...
        <p style={{ color: '#64748b', marginTop: 8 }}>Input daily shift data and manage surge capacity.</p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(350px, 1fr))', gap: 24 }}>
        ...
        <div 
          ...
          style={{ 
            ...
            borderRadius: 20, 
            padding: 24, 
            border: latestSurge.count > 0 ? '1px solid #f59e0b' : '1px solid #e2e8f0', // Orange border if active
            boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)', 
            position: 'relative',
            overflow: 'hidden',
            cursor: 'pointer',
      ...
      {/* --- MODAL 2: SURGE CAPACITY INPUT (MODIFIED) --- */}
      {showSurgeModal && (
        <SurgeCapacityModal 
          ward={wardInfo} 
...
// ==============================================
// SUB-COMPONENT: SURGE CAPACITY MODAL (WITH CONFIRMATION)
// ==============================================
    ...
    const getCapacity = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // Filter specifically for the selected ward (Ward A)
